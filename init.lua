...
local function waitForMainWindow(appNameOrBundle, callback)
    ...
    local function attempt()
        ...
        if not app then
            log("waitForMainWindow: app not found (%s), retries left=%d",
                appNameOrBundle, retries) -- LOG
            retries = retries - 1
            if retries > 0 then
        ...
        log("waitForMainWindow: no windows yet for %s, retries left=%d",
            appNameOrBundle, retries) -- LOG
        retries = retries - 1
        if retries > 0 then
            hs.timer.doAfter(config.launch.windowRetryInterval, attempt)
...
for i, appName in ipairs(workspaceApps) do
    hs.hotkey.bind({"alt", "shift"}, tostring(i), function()
        log("hotkey: alt+shift+%d → %s", i, appName)
        local front = hs.application.frontmostApplication()
        if front then
            local frontBundle = front:bundleID()
            if frontBundle and frontBundle ~= "" then
                lastApp = frontBundle
                log("hotkey: saving lastApp (bundle) → %s", lastApp)
            else
                lastApp = front:name()
                log("hotkey: saving lastApp (name) → %s", lastApp)
            end
        end
        launchApp(appName)
    end)
...
hs.hotkey.bind({"alt"}, "escape", function()
    ...
    if bundle and bundle ~= "" then
        lastApp = bundle
        log("alt+esc: saving lastApp (bundle) = %s", lastApp)
    else
        local name = current and current:name()
        lastApp = name
        log("alt+esc: saving lastApp (name) = %s", tostring(lastApp))
    end

    -- Switch to terminal by bundle ID (deterministic)
    log("alt+esc: switching to terminal bundle=com.mitchellh.ghostty")
